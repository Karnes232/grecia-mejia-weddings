...
export const seo = defineType({
  ...
  fields: [
    defineField({
      ...
      type: "string",
      validation: (Rule) =>
        Rule.max(60).warning("Should be 60 characters or less"),
    }),
    defineField({
      name: "description",
      ...
      validation: (Rule) =>
        Rule.required()
          .min(50)
          .max(160)
          .warning("50–160 characters recommended"),
    }),
    defineField({
      name: "ogImage",
...
export const destination = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      of: [{ type: "faq" }],
      validation: (Rule) =>
        Rule.min(4).warning("At least 4 FAQs recommended for FAQ schema"),
    }),
...
// ✅ Tight
client.fetch(
  `*[_type == "article" && language == $locale] | order(publishedAt desc) [0...12] {
  ${articleCardFragment}
}`,
  { locale },
);
