...
const ATELIER: Record<
  Locale,
  { eyebrow: string; headline: string; paragraphs: string[]; signature: string }
> = {
  en: {
...
const CULTURES: Record<
  Locale,
  Record<(typeof CULTURE_SLUGS)[number], { name: string; description: string }>
> = {
  en: {
    "indian-weddings": {
...
const VENUES_DATA = [
  ...
  {
    slug: "chateau-de-la-croix",
    name: "Château de la Croix",
    region: "Provence",
  },
  { slug: "finca-san-bartolo", name: "Finca San Bartolo", region: "Mallorca" },
  {
    slug: "castello-di-velasco",
    name: "Castello di Velasco",
    region: "Tuscany",
  },
] as const;

const VENUE_DESCRIPTIONS: Record<
  Locale,
  Record<(typeof VENUES_DATA)[number]["slug"], string>
> = {
  en: {
    "jellyfish-restaurant":
      "Open-air pavilion on the shoreline; intimate dinners under canvas.",
    ...
    "castello-di-velasco":
      "Renaissance castle, frescoed ceilings, vineyard terraces.",
  },
  es: {
    "jellyfish-restaurant":
      "Pabellón al aire libre sobre la orilla; cenas íntimas bajo lona.",
    "kukua-beach-club":
      "Arena blanca, linos en tono oliva y un horizonte largo.",
    ...
    "castello-di-velasco":
      "Castillo renacentista, techos al fresco, terrazas de viñedo.",
  },
  fr: {
    "jellyfish-restaurant":
      "Pavillon ouvert sur le rivage ; dîners intimes sous toile.",
    ...
    "castello-di-velasco":
      "Château Renaissance, plafonds peints, terrasses de vignoble.",
  },
  pt: {
    "jellyfish-restaurant":
      "Pavilhão aberto sobre a orla; jantares íntimos sob lona.",
    "kukua-beach-club":
      "Areia branca, linhos em tom oliva e um horizonte longo.",
    ...
    "castello-di-velasco":
      "Castelo renascentista, tetos afrescados, terraços de vinhedo.",
  },
  de: {
    "jellyfish-restaurant":
      "Offener Pavillon am Ufer; intime Dinner unter Segeltuch.",
    "kukua-beach-club":
      "Weißer Sand, olivfarbene Leinen und ein langer Horizont.",
    "chez-bisutti": "Anwesen auf der Höhe, Zitrushaine, eine Privatkapelle.",
    "villa-ta-ville":
      "Italienisch-modernistische Villa mit Palisanderinterieurs.",
    "chateau-de-la-croix":
      "Provenzalisches Château, eingerahmt von Lavendelfeldern.",
    "finca-san-bartolo": "Steinerne Finca, alte Olivenbäume, Meereslicht.",
    "castello-di-velasco":
      "Renaissance-Schloss, freskenverzierte Decken, Weinbergterrassen.",
  },
  it: {
    "jellyfish-restaurant":
      "Padiglione aperto sulla riva; cene intime sotto tela.",
    ...
    "castello-di-velasco":
      "Castello rinascimentale, soffitti affrescati, terrazze di vigneto.",
  },
};
...
async function run() {
  ...
  await tx.commit();
  console.log(
    `✓ Seeded ${locales.length} homePage docs + translation metadata.`,
  );
}
