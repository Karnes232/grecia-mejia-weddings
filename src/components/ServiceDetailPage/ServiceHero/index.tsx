...
type ServiceHeroProps = {
  hero: NonNullable<Service["hero"]>;
  image: NonNullable<Service["media"]>["heroImage"];
};

export function ServiceHero({ hero, image }: ServiceHeroProps) {
  ...
  return (
    <section className="relative isolate h-[78vh] min-h-[680px] max-h-[880px] overflow-hidden bg-cine-bg">
        ...
        <div className="absolute -inset-[4%] z-0">
          <div className="absolute inset-0 motion-safe:[animation:ken-burns_28s_cubic-bezier(0.45,0.05,0.55,0.95)_infinite_alternate]">
            <Image
              ...
              style={{ filter: "saturate(0.85) contrast(1.04) brightness(0.68)" }}
            />
          </div>
        </div>
      ) : null}
      ...
      <div className="relative z-[2] flex h-full flex-col items-center justify-center px-[7vw] pt-[120px] pb-[140px] text-center text-ivory">
        ...
        {hero.headline ? (
          <h1 className="m-0 font-serif font-light italic text-ivory text-[clamp(64px,10vw,132px)] leading-[0.92] tracking-[-0.018em]">
            {renderHeadline(hero.headline, "gold")}
          </h1>
        ) : null}

        {hero.deck ? (
          <p className="mx-auto mt-12 max-w-[56ch] font-serif italic text-[21px] leading-[1.5] text-cine-ivory">
            {hero.deck}
          </p>
        ) : null}
      </div>
